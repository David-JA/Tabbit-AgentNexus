...
默认将“一次可提交的 git 变更集”视为一个阶段工作。

- 当 agent 判断当前工作已具备提交条件，但仍需先征求用户是否提交时，应先按阶段报告格式汇报当前阶段状态，再询问是否执行 `git commit`。
- 当用户显式要求 agent 执行 `git commit` 时，默认视为用户已认定当前阶段工作应收口；agent 完成提交后，仍应按阶段报告格式汇报本次提交涉及的实现、验证、限制和下一步。
- 如果当前只是中途同步、草稿讨论或明确不进入提交语义，可以不强制输出完整阶段报告；但只要进入“可提交 / 待提交确认 / 已提交”的语义，就默认触发阶段报告。
